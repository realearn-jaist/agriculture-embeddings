...
class RiceDOReasonData:
  ...
  def __init__(self, path, negSampleSizeRatio, mapLoadPath=None, mapLoadNamePrefix=None):
    ...
    if mapLoadPath==None and mapLoadNamePrefix==None:
      self.e2id, self.uc2id, self.bc2id = self.getEntitiesConceptsMap(path+'/train.tsv')
    else:
    ...
    self.aboxUTripleLst, self.aboxBTripleLst = self.readAboxTripleList(path+'/train.tsv')
    ...
    self.allAboxUTripleLst, self.allAboxBTripleLst = self.readAllAboxTripleList(path+'/train.tsv', path+'/validate.tsv', path+'/test.tsv')
    self.allUCMemberMap, self.allBCMemberMap = self.getAllClassMemberMap()
    self.testAboxUTripleLst, self.testAboxBTripleLst = self.readAboxTripleList(path+'/test.tsv')
  ...
  def getTrainDataTensor(self, batchIndex, batchSize):
    """
    one must imagine that relation consist of (head, relationship, tail)
    aBHELst is a list contain only index of a Head Entity
    aBCLst is a list contain only index of a relationship
    aBTELst is a list contain only index of a Tail Entity
    n prefix is for negative sample?
    """
    ...
    nAUELst = []
    nAUCLst = []
    aBHELst = [] # contain head
    aBTELst = [] # contain tail
    aBCLst = []
    nABHELst = []
    nABTELst = []
    nABCLst = []
    ...
    uniqELst = list(self.id2e.keys())
    uniqUCLst = list(self.id2uc.keys())
    uniqBCLst = list(self.id2bc.keys())

    return (
      torch.LongTensor(aUELst), torch.LongTensor(aUCLst),
      torch.LongTensor(nAUELst), torch.LongTensor(nAUCLst),
      torch.LongTensor(aBHELst), torch.LongTensor(aBTELst), torch.LongTensor(aBCLst), 
      torch.LongTensor(nABHELst), torch.LongTensor(nABTELst), torch.LongTensor(nABCLst), 
      torch.LongTensor(uniqELst), torch.LongTensor(uniqUCLst), torch.LongTensor(uniqBCLst)
    )
...
if __name__ == '__main__':
  ...
  #from data import FB15kData

  # dataPath = '/u/sha/resources/data/kg-embed/fb15k/FB15k/'
  dataPath = './data'
  data = FB15KReasonData(dataPath,3)
  print(random.sample(data.aboxBTripleLst, 10))
  
  print(data.bc2id)
  print(data.tboxBTripleLst)
  # print(data.negTboxUTripleLst)
